    ...
    if form.submit.data and form.validate_on_submit() and dynamic_form.validate():
        ...
        for field in dynamic_form:
            if field.type == "TextField":
                ...
                template.field.append(Template_field(label = label, text = field.data)) 
            elif field.type == "DateField":
                template.field.append(Template_field(label = label, date = field.data))
            elif field.type == "FileField":
                template.field.append(Template_field(label = label, filename = "1"))
